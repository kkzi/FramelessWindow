...
namespace QWK
{
    ...
    static inline void bringWindowToFront(HWND hwnd)
    {
        ...
        if (!::SendMessageTimeoutW(oldForegroundWindow, WM_NULL, 0, 0, SMTO_BLOCK | SMTO_ABORTIFHUNG | SMTO_NOTIMEOUTIFNOTHUNG, 1000, nullptr))
        {
    ...
    static bool showSystemMenu_sys(HWND hWnd, const POINT &pos, const bool selectFirstEntry, const bool fixedSize)
    {
        ...
        const auto result = ::TrackPopupMenu(
            hMenu, (TPM_RETURNCMD | (QGuiApplication::isRightToLeft() ? TPM_RIGHTALIGN : TPM_LEFTALIGN) | TPM_RIGHTBUTTON), pos.x, pos.y, 0, hWnd, nullptr);
    ...
    {
        if (((message >= WM_NCCREATE) && (message <= WM_NCACTIVATE)) || ((message >= WM_NCMOUSEMOVE) && (message <= WM_NCMBUTTONDBLCLK)) ||
    ...
    static inline constexpr bool isInputMessage(UINT m)
    {
        ...
        return (m >= WM_MOUSEFIRST && m <= WM_MOUSELAST) || (m >= WM_NCMOUSEMOVE && m <= WM_NCXBUTTONDBLCLK) || (m >= WM_KEYFIRST && m <= WM_KEYLAST);
    }
    ...
    static inline bool forwardFilteredEvent(QWindow *window, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *result)
    {
    ...
    extern "C" LRESULT QT_WIN_CALLBACK QWKHookedWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (ctx->windowProc(hWnd, message, wParam, lParam, &result))
        {
            ...
            std::ignore = forwardFilteredEvent(ctx->window(), hWnd, message, wParam, lParam, &result);
            return result;
        }
    ...
    QVariant Win32WindowContext::windowAttribute(const QString &key) const
    {
        ...
        {
            return m_windowId ? int(getWindowFrameBorderThickness(reinterpret_cast<HWND>(m_windowId))) : 0;
        }
    ...
    bool Win32WindowContext::windowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *result)
    {
        *result = FALSE;
    ...
    bool Win32WindowContext::windowAttributeChanged(const QString &key, const QVariant &attribute, const QVariant &oldAttribute)
    {
    ...
    {
        quint64 result = 0;
        const auto &get = [](const int virtualKey) -> bool { return (::GetAsyncKeyState(virtualKey) < 0); };
    ...
    static void emulateClientAreaMessage(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, const std::optional<int> &overrideMessage = std::nullopt)
    {
...
#undef SEND_MESSAGE
    ...
    bool Win32WindowContext::snapLayoutHandler(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *result)
    {
        switch (message)
        {
        case WM_MOUSELEAVE: {
            if (wParam != kMessageTag.wParam)
            {
                ...
                QPoint qtScenePos = QHighDpi::fromNativeLocalPosition(point2qpoint(screenPoint), m_windowHandle.data());
    ...
    bool Win32WindowContext::customWindowHandler(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *result)
    {
        switch (message)
        {
        case WM_NCHITTEST: {
            ...
            QPoint qtScenePos = QHighDpi::fromNativeLocalPosition(point2qpoint(nativeLocalPos), m_windowHandle.data());
    ...
    bool Win32WindowContext::nonClientCalcSizeHandler(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *result)
    {
        Q_UNUSED(message)
        Q_UNUSED(this)
        ...
        const auto clientRect = wParam ? &(reinterpret_cast<LPNCCALCSIZE_PARAMS>(lParam))->rgrc[0] : reinterpret_cast<LPRECT>(lParam);
        if (isSystemBorderEnabled())
        {
    ...
    bool Win32WindowContext::systemMenuHandler(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT *result)
    {
        if (m_delegate->getWindowFlags(m_host).testFlag(Qt::Dialog))
        {
            return false;
        }
        const auto getNativePosFromMouse = [lParam]() -> POINT { return { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) }; };
        ...
        case WM_RBUTTONUP: {
            ...
            const QPoint qtScenePos = QHighDpi::fromNativeLocalPosition(point2qpoint(nativeLocalPos), m_windowHandle.data());
            WindowAgentBase::SystemButton sysButtonType = WindowAgentBase::Unknown;
            if (isInTitleBarDraggableArea(qtScenePos) || (isInSystemButtons(qtScenePos, &sysButtonType) && sysButtonType == WindowAgentBase::WindowIcon))
            {
        ...
        if (shouldShowSystemMenu)
        {
            ...
            bool res = showSystemMenu_sys(hWnd, nativeGlobalPos, broughtByKeyboard, isHostSizeFixed());
            ...
            if (mouseHookedLocal)
            {
                ...
                if (!res && mouseClickPos.has_value())
                {
                    ...
                    QPoint qtScenePos = QHighDpi::fromNativeLocalPosition(point2qpoint(nativeLocalPos), m_windowHandle.data());
                    WindowAgentBase::SystemButton sysButtonType = WindowAgentBase::Unknown;
                    if (isInSystemButtons(qtScenePos, &sysButtonType) && sysButtonType == WindowAgentBase::WindowIcon)
                    {
